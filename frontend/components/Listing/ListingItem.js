...
function ListingItem({ idx, publicKey, connected, showReservedListing, location, country, distance, price, rating, image, isReserved, reservation, removeListing, toggleEditListingModal, toggleReserveListingModal, unreserveListing }) {
    ...
    return (
        <div className="flex flex-col space-y-3 cursor-pointer">
            <div className="relative h-64 w-auto group">
                ...
                {connected && (
                    <div className="opacity-0 group-hover:opacity-100 transition-all duration-150 absolute top-4 right-4 flex space-x-2">
                        <PencilIcon onClick={() => toggleEditListingModal(true,idx)} className="w-6 h-6 text-white opacity-80" />
            ...
            <div>
                ...
                <p className="text-sm font-light text-gray-600">{formatNumber("788")} kilometers</p>
